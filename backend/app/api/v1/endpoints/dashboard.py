...
# C@cache(expire=300) # 5 minutos de caché
# Force reload for natural sort fix
def get_funnel_stats(
    data_service: DataService = Depends(get_data_service)
):
    """
    Obtiene estadísticas del funnel de donantes y aprobaciones pendientes.
    """
    try:
        return data_service.get_funnel_stats()
    except Exception as e:
        print(f"Error crítico al obtener estadísticas del funnel: {e}")
